...
const ProductForm = ({ typeAction, typeProduct }) => {
  ...
  useEffect(() => {
    ...
    if (typeAction === "edit")
      getProductDetailAdmin(id, form, setSizes, handleFileListChange);
  }, [id, typeAction, form]);
      ...
      <Row>
        <Col offset={4} span={12}>
          ...
          >
            ...
            {typeAction === "create" &&
              (typeProduct === "tshirt" || typeProduct === "pant") && (
                <TshirtSize error={error} />
              )}
            {typeAction === "create" && typeProduct === "shoes" && (
              <ShoesSize error={error} />
            )}
            {typeAction === "create" && typeProduct === "racket" && (
              <RacketSize error={error} />
            )}
